    ...
    if (response.data && Array.isArray(response.data) && response.data.length > 0) {
      ...
      cateMap.forEach((items, cateName) => {
        ...
        if (existingCate) {
          ...
          const toolList = items.map((item: any, index: number) => ({
            id: index + 1,
            title: item.title,
            logo: item.logo || '/images/logo/jump.jpg',
            desc: item.description,
      ...
      mergedResult.sort((a, b) => {
        ...
        return getSortKey(a.title) - getSortKey(b.title);
      });
...
// 本地数据备份，用于API不可用时的回退
export const localToolsCate: ToolCate[] = [
  {
      id: 0,
      title: '00 自用',
      icon: '',
